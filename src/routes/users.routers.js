...
router.post("/users/sign-up", async (req, res, next) => {
  try {
    ...
    const [Users, UserAccount, UserElo] = await prisma.$transaction(
      async (tx) => {
        try {
          // 유저 테이블 추가
...
router.get("/users/ranks", authMiddleware, async (req, res, next) => {
  ...
  try {
    ...
    if (!userElo)
      return res
        .status(404)
        .json({ message: "[Not Found] 유저의 랭킹 정보 찾을 수 없음." });
...
router.get("/users/ranks/top", async (req, res, next) => {
  ...
  try {
    ...
    if (topRankers.length === 0)
      return res
        .status(404)
        .json({ message: "[Not Found] 그 어느 랭커가 존재하지 않음." });
